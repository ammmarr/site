...
export const PrivateTripOneRoundId: FC<RentalCarDatesRangeInputProps> = ({
  defaultDateValue,
  onChange,
  defaultFocus = null,
  onFocusChange,
  ...
  const date:any = window.localStorage.getItem("private_oneRound_date_time");
  
  const private_oneRound_date_time = date.split("T")[0].substring(1,11)
  
  console.log(private_oneRound_date_time)
    ...
    if(dropOffLocationType === "oneWay") {
      if (
        ...
        const OneRoundPost = {
          round: 1,
          boarding: {
            date: `${private_oneRound_date_time} ${time}`,
            from_address_id: AddressFromOne?.id,
            to_address_id: AddressToOne?.id
          }
        };
        ...
        axios
          .post(
            ...
            `https://app.telefreik.com/api/v2/transports/private/trips/${trip_Id}/summary`,
            OneRoundPost,
            {
              headers: {
        ...
        nav("/private-trip/oneRound/summary");
      }
    } 
