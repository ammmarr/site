...
function getmax(num: any[]): any | undefined {

  return Math.max(...num);
}
function getmin(num: any[]): any | undefined {

  return Math.min(...num);
}

function ProcessTripsPrices(tripsData: any[]): any[] {
  let pricesoftrips: any[] = [];
  tripsData?.map((tripData) => {
    pricesoftrips.push(tripData);
  });

  return pricesoftrips;
}
...
export const PrivateTrip: FC<PrivateTripPror> = () => {
  ...
  const [minValue, setMinValue] :number | string | any  = useState(MIN);
  const [maxValue, setMaxValue] :number | string | any = useState(MAX);

 useEffect(() => {
  let price : number[] = [] 
  price =  data?.map((item : any) => { return parseInt(item.price) })
  let min = getmin(ProcessTripsPrices(price))
  let max = getmax(ProcessTripsPrices(price))
   setMin(min)
   setMax(max)
 } , [data ])
  ...
  const [screenSize, setScreenSize] = useState(getCurrentDimension());
    ...
    const max_and_medium_screen = (data: Item[]): JSX.Element[] => {
      return data?.map((item) => (
        <div
          className="mt-5 flex w-[100%] flex-col items-center rounded-lg bg-[white]"
          key={item.id}
        >
             ...
             >
               <div className="mt-4 flex h-[auto] w-full items-center justify-around  pb-3 pt-3">
                 ...
                 <div className="m-[auto] rtl:m-[auto] ml-10 flex  flex-col ">
...
<div className={`container w-[90%] mb-5 mt-[8%] max-sm:mt-[10%] ${Styled.container_private}`} >
     ...
     <div className="flex w-[73%]  flex-col items-between sm:w-full max-sm:w-full ">
